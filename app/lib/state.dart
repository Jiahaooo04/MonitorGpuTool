...
List<int> keyFromB64(String s) => base64Url.decode(
      ...
      switch (msg['t'] as String?) {
        ...
        case 'snapshot':
          ...
          if (data['server_now'] is num) {
            agent.clockOffset =
                (data['server_now'] as num).toDouble() -
  ...
  Future<String?> pairWithPayload(
    ...
    try {
      ...
      final link = ServerLink(
        ...
        appDeviceId: claim['device_id'],
        appToken: claim['device_token'],
...
String fmtDuration(double? startedAt, double? endedAt, [double? nowOverride]) {
  ...
  final end =
      endedAt ?? nowOverride ?? DateTime.now().millisecondsSinceEpoch / 1000;
