...
- 📈 **Resource curves & GPU owners** — GPU utilization / VRAM / CPU / memory as live charts; tap any GPU to see the user, PID, process, CPU, RAM, and VRAM behind it
...
- 🧠 **Plain-language failure summaries** — configure DeepSeek / Qwen / Kimi / OpenAI / Ollama per server in the app; related error and exit notices merge into one diagnosis, with automatic fallback when the model API is unavailable
...
- LLM API keys travel over the end-to-end encrypted channel and remain only on the selected server (`0600`); neither the app nor relay reads them back, and common credentials are redacted from logs before sending
