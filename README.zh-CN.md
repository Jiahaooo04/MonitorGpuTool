...
- 📈 **资源曲线 & GPU 占用者** —— GPU 利用率 / 显存 / CPU / 内存实时曲线;点任意 GPU 可查看用户、PID、进程名及其 CPU / 内存 / 显存占用
...
- 🧠 **报错总结成人话** —— 每台服务器可在 App 里单独选择 DeepSeek / 通义 / Kimi / OpenAI / Ollama;同一次报错合并为一条分析通知,接口出错自动退回普通通知
...
- LLM API Key 经端到端加密下发后只保存在对应服务器(`0600`),App 和 relay 都不会读取明文;发送前自动遮盖日志中的常见凭证
