...
def _write_fake_apkanalyzer(
    ...
    *,
    ...
    path.write_text(
        ...
        '  version-code)\n'
        '    case "$name" in\n'
        '      first.apk|RunMon.apk) echo "6" ;;\n'
        '      second.apk|RunMon-arm64.apk) echo "2006" ;;\n'
        '      RunMon-arm32.apk) echo "1006" ;;\n'
        '      RunMon-x86.apk) echo "4006" ;;\n'
        '      *) exit 2 ;;\n'
        '    esac ;;\n'
        "  *) exit 2 ;;\n"
        "esac\n",
...
def _fixture(
    ...
    *,
    ...
    pubspec.write_text("name: runmon_app\nversion: 1.0.7+6\n", encoding="utf-8")
...
def _run(paths: dict[str, Path], *extra: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            ...
            str(paths["output"]),
            "--url",
            "https://example.test/RunMon-arm64.apk",
            "--base-apk",
            str(paths["first_apk"]),
            "--update-apk",
...
def test_uses_real_version_code_from_the_update_apk(tmp_path: Path) -> None:
    ...
    assert json.loads(paths["output"].read_text(encoding="utf-8")) == {
        ...
        "versionCode": 2006,
        "notes": "· 新增 AI 报错总结\n· 新增 GPU 占用进程详情",
        "url": "https://example.test/RunMon-arm64.apk",
    }
