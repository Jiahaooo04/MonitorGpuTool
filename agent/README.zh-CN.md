...
| 🤫 日志静默 | 30 分钟无新输出 | 警告 |
| 💾 磁盘告警 | 任一挂载点默认达到 90%(App 可调 50%～99%) | 警告 |
...
disk_threshold_pct = 90    # 磁盘告警阈值(50～99)
```

也可以在 App 的「设置 → 磁盘告警阈值」中调整。保存后会下发给所有在线服务器,
并持久化到各服务器的 Agent 配置中。

### 可选:用 LLM 把报错总结成人话

启用后,`mon run` 检测到 Traceback / CUDA OOM / Segfault 或任务失败时,会先让你自己的
OpenAI 兼容接口把日志尾部总结成一两句话,并把错误输出和退出状态合并成一条通知。
默认关闭;接口超时、失败或未配置时自动退回附带简短日志末尾的普通通知,不会影响通知必达。

最简单的配置方式是在 App 中打开对应服务器,点右上角的「报错总结」按钮:选择供应商和
模型、填入 API Key,先点「测试连接」,成功后再保存。配置按服务器分别保存;Key 经
端到端加密发给 Agent,Relay 看不到明文,App 重新打开时也不会把 Key 从服务器取回。
